...
# Function to draw red flowers along the boundary of the heart
def draw_flower(x, y):
    turtle.penup()
    turtle.goto(x, y)
    turtle.pendown()
    turtle.color("red")  # Change flower color to red
    turtle.begin_fill()
    for _ in range(6):
        turtle.circle(10, 60)
        turtle.left(120)
...
def handle_click(x, y):
    global is_open
    if not is_open:
        # Draw flowers along the boundary of the heart
        for angle in range(0, 360, 30):
            x = 150 * math.cos(math.radians(angle))
            y = 150 * math.sin(math.radians(angle))
            draw_flower(x, y)
        # Open the heart and display the message
        turtle.clear()
        draw_heart()
        display_message()
    else:
        # Clear the message and redraw the heart
        clear_message()
        draw_heart()
        # Draw flowers along the boundary of the heart
        for angle in range(0, 360, 30):
